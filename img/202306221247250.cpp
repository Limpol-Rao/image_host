...
void test()
{
    ...
    for (int i = 0; i < 10; i++)
    {
        double precision = true_positive[i] * 1.0 / (true_positive[i] + false_positive[i]);
        double recall = true_positive[i] * 1.0 / (true_positive[i] + false_negative[i]);
    ...
    cout << "epoch = " << e << ", average precision = " << total_precision / 10.0
         ...
         << ", time:" << chrono::duration_cast<chrono::seconds>(chrono::steady_clock::now() - start_time).count() << endl;
